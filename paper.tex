...
& (28 strains) & (5 strains,  & (2 strains) \\
& & many isolates) & \\ \midrule 
number of HP PPIs (positives) & 848& 981 & 90 \\\hline
number of non-homologous PPIs & 334 & 329 & 49 \\ \hline
...
\multicolumn{4}{l}{\scriptsize{$\ddagger$ Density = (no. of positives + no. of negatives)/ total number of possible edges. The}} \\
\multicolumn{4}{l}{\scriptsize{\hspace{0.1cm} denominator was computed considering all proteins from all strains of the virus and}}\\
\multicolumn{4}{l}{\scriptsize{\hspace{0.1cm} $\approx$ 100,000 human proteins.}}\\
\multicolumn{4}{l}{\scriptsize{$\dagger$ this is the number of `reviewed' proteins on UniprotKB from all strains in our data}} \\
\bottomrule
\end{tabular}
\end{footnotesize}
}
{
\caption{Tasks and their sizes. Each column corresponds to one bipartite graph between human proteins and the pathogen indicated in the column header. All pathogens are single stranded RNA viruses. The last row shows that each of our graphs is extremely sparse.} 
\label{datasets}
}
\end{floatrow}
\end{figure*}
...
\section{Results}
As described in Section \ref{}, the PPI data we obtain from PHISTO has homologs within a task. We therefore present results on two sets of data: (a) with the original PPIs from PHISTO shown in Table \ref{resultsTable} and (b) after removal of all homologs within each task in Table \ref{homresultsTable}. The latter comparison gives us an idea of how much the approaches benefit from the redundancy within each task.
...
\begin{table}[t]\caption{Area Under the Precision-Recall curve for each task upon removal of the homologs within each task, on the 10\% setting}
...
Homolog & & & \\
STL (Ridge Reg.)  & & & \\
Sparse+low-rank \citep{chen2012} & & &  \\ 
BSL-MTL (this work) & & &  \\ \bottomrule
\end{tabular}
\end{center}
\end{small}
\end{table}
...
{
\caption{Hepatitis-human PPI}
\label{fig:flavinet}
}
\ffigbox[0.34\textwidth]{%
%trim option's parameter order: left bottom right top
\includegraphics[scale=0.35, trim = 0.2 0.2 0.2 0.2]{flunet.pdf}
}
{
\caption{Influenza-human PPI}
\label{fig:flunet}
}
\end{subfloatrow}
}
{
\caption{PPI from each task that are found by our method but are missed by the homolog method}
}
\end{figure*}
...
%trim option's parameter order: left bottom right top
\includegraphics[scale=0.35, trim = 0 0.8cm 0 0]{fig2.eps}
}{%
\caption{Principal component analysis (PCA) of virus proteins in the original feature space}%
\label{fig:pca1}
}
\ffigbox[0.49\columnwidth]{%
\includegraphics[scale=0.33, trim = 0 0 0 0]{fig3.eps}
}{%
\caption{Principal component analysis (PCA) of virus proteins in the projected subspace. The first two principal components are shown. Shape of the points indicates which virus that protein comes from.}%
\label{fig:pca2}
}
\end{floatrow}
\end{figure}
