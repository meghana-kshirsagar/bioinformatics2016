...
For reference, the AUC-PR of a random classifier model is $\approx 0.01$ due to the class imbalanced nature of the data (class skew = 1:100).
...
Results on the homolog part of our dataset sheds further light on this.

\noindent\textit{Performance on the non-homologous PPI partition}:
In Table \ref{nohomresultsTable} we show AUC for all the representative baselines in the 10\% setting. 
Overall, our method performs best on this partition over all tasks.
...
and demonstrates that we are able to predict such PPI by intelligently sharing information with other tasks. 
...
\begin{center}
\includegraphics[scale=0.28, trim = 0.5cm 0.5cm 0.5cm 0.5cm]{flavinet.pdf}
\end{center}
\end{minipage}
\\
\begin{minipage}{\textwidth}
...
\begin{figure}
\centering
\includegraphics[scale=0.4, trim = 0 0.2cm 0 0.5cm]{fig2-eps-converted-to.pdf}
...
of interaction are biologically interesting.
...
The method was evaluated in the host-pathogen protein interaction domain for three pathogens (three tasks) and exhibited large increases in prediction accuracy. The model parameters provide several avenues for
