...
\begin{algorithm}[!b]   \caption{Alternating Least Squares}
   \label{alg}
...
%trim option's parameter order: left bottom right top
\includegraphics[scale=0.25, trim = 0 0 0 0]{top10ebola_3mers_seq2logo.pdf}
}{%
...
\ffigbox[0.4\columnwidth]{%
%trim option's parameter order: left bottom right top
\includegraphics[scale=0.25, trim = 0 0 0 0]{top10flu_3mers_seq2logo.pdf}
}{%
\caption{Sequence motifs specific to \textit{Influenza} proteins that are also important to interactions.}%
\label{fig:flu_motifs}
}
